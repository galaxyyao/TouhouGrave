    ...
    public partial class Game
    {
        ...
        internal CommandResult ResolveConditions(bool prerequisiteOnly)
        {
            for (int i = 0; i < Players.Count; ++i)
            {
                ...
                m_manaNeeded[i] = player.CalculateFinalManaSubtract(m_manaNeeded[i]);
            ...
            if (!prerequisiteOnly)
            {
                ...
                for (int i = 0; i < Players.Count; ++i)
                {
                    ...
                    {
                        IssueCommand(new Commands.SubtractPlayerMana(Players[i], Players[i].Mana, true, null));
                    }
                    else if (m_manaNeeded[i] != 0)
                    {
                        IssueCommand(new Commands.SubtractPlayerMana(Players[i], m_manaNeeded[i], true, null));
                    }
                }
            }
